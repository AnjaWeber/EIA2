...
namespace BeachScenePolymorphyAnimationAnja {
    ...
    export class Moewe extends BeachElement {
        ...
        constructor(_position: Vector, _size: Vector, _velocity: Vector) {
            let velocity: Vector = new Vector((Math.random() < 0.5 ? -1 : 1) * Math.random() * _velocity.x, (Math.random() < 0.5 ? -1 : 1) * Math.random() * _velocity.y);
            super(_position, velocity);
        ...
        move(_timeslice: number): void {
            ...
            if (this.movementCounter >= this.changeOfDirectionCount) {
                ...
                if (Math.random() < 0.5) {
                    this.velocity = new Vector((Math.random() < 0.5 ? -1 : 1) * Math.random() * this.originalVelocity.x, (Math.random() < 0.5 ? -1 : 1) * Math.random() * this.originalVelocity.y);
                }
            }
